...
⚠️ IMPORTANT: All flashcard values (questions and answers) must be in the Georgian language.
Keep the JSON keys in English, only translate the values.

Example format:
{{
  "flashcards": [
    {{"id": 1, "front": "რა არის ხელოვნური ინტელექტი?", "back": "ხელოვნური ინტელექტი არის ..."}}
  ]
}}
"""),
    ...
    async def generate_questions(self, state: RAGState) -> RAGState:
        try:
            ...
            quiz_prompt = ChatPromptTemplate.from_messages([
                ("system", system_prompt + """
Create a quiz of 10 multiple-choice questions in JSON.
⚠️ IMPORTANT: All question text, answer options, and explanations must be in the Georgian language.
Keep the JSON keys in English, only translate the values.
...
{{
  "questions": [
    {{
      "id": 1,
      "question": "როდის შეიქმნა პირველი ჩათბოტი ELIZA?",
      "options": {{"A": "1950", "B": "1960-იანი წლები", "C": "1977", "D": "2011"}},
      "correct_answer": "B",
      "explanation": "ELIZA შეიქმნა 1960-იან წლებში როგორც პირველი ჩათბოტი."
    }}
  ]
}}
"""),
