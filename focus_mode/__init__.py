            ...
            try:
                ...
                for proc in psutil.process_iter(['pid', 'name']):
                    try:
                        ...
                        if any(blocked_app.replace('.exe', '').lower() in process_name for blocked_app in self.blocked_apps):
...
class FocusModeGUI:
    ...
    def create_checkboxes(self, parent):
        ...
        categories = {
            ...
            "📱 სოციალური მედია": ["facebook.exe", "instagram.exe", "linkedin.exe", "pinterest.exe","snapchat.exe", "tiktok.exe"],
            "🎮 გეიმები": ["roblox.exe", "minecraft.exe", "fortnite.exe", "valorant.exe","csgo.exe", "leagueoflegends.exe", "dota2.exe", "overwatch.exe", "callofduty.exe", "pubg.exe", "warzone.exe", "fifa.exe"]
        }
